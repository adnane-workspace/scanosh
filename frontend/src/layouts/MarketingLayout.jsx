...
export default function MarketingLayout({ children }) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-on-surface">
      <header
        className={`pointer-events-none fixed inset-x-3 top-[max(0.7rem,env(safe-area-inset-top))] z-40 sm:inset-x-4 sm:top-[max(0.9rem,env(safe-area-inset-top))] ${
          menuOpen ? 'invisible' : ''
        }`}
      >
        <nav
          className={`pointer-events-auto relative mx-auto max-w-[1080px] rounded-full border backdrop-blur-xl transition-all duration-300 ${
            onDarkNav
              ? `border-white/12 bg-[#0d1b2a]/55 ${scrolled ? 'shadow-[0_10px_40px_rgba(0,0,0,0.38)]' : 'shadow-[0_8px_28px_rgba(0,0,0,0.22)]'}`
              : 'border-on-surface/10 bg-background/80 shadow-[0_8px_28px_rgba(13,27,42,0.08)]'
          }`}
        >
          <div className="flex h-12 items-center justify-between gap-2 px-1.5 sm:h-[3.25rem] sm:gap-3 sm:px-2">
            <MarketingLink
              to="/"
              className="relative z-20 flex min-w-0 shrink items-center rounded-full px-2.5 py-1"
              aria-label={APP_NAME}
              onClick={() => {
                setMenuOpen(false);
                if (path === '/') {
            ...
            <div
              className={`flex shrink-0 items-center gap-0.5 p-0.5 sm:gap-1 ${
                isHome ? (onDarkNav ? 'rounded-full bg-white/6 ring-1 ring-white/8' : 'rounded-full bg-on-surface/4') : ''
              }`}
            >
              <LanguageSwitcher compact onDark={onDarkNav} className={isHome ? 'inline-flex' : 'hidden md:inline-flex'} />
              <AppLink
                to="/login"
                className={`inline-flex items-center rounded-full px-2 py-1.5 text-[13px] font-medium whitespace-nowrap transition-colors sm:px-3 sm:text-sm ${
                  onDarkNav ? 'text-white/80 hover:bg-white/10 hover:text-white' : 'text-on-surface/70 hover:bg-on-surface/5 hover:text-on-surface'
                }`}
              >
                {t('landing.ctaLogin')}
              </AppLink>
              ...
              {!isHome ? (
                <button
                  type="button"
                  className={`inline-flex h-9 w-9 items-center justify-center rounded-full xl:hidden ${
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4">
            ...
            <div>
              ...
              <ul className="flex flex-col gap-4 text-on-surface-variant">
                ...
                <li>
                  <Link to={LANDING_PRIVACY} className="hover:text-on-surface">
                    {t('landing.privacy')}
                  </Link>
                </li>
              </ul>
          ...
          <div className="flex flex-col gap-4 border-t border-on-surface/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-center sm:text-start">
              ...
              <p className="mt-2 text-label-md text-on-surface-variant">
                ...
                {' · '}
                <Link to={LANDING_PRIVACY} className="text-on-surface hover:underline">
                  {t('landing.privacy')}
                </Link>
              </p>
            </div>
